...
public class PileLayout extends ViewGroup {
    ...
    public void setSizeRatio(float sizeRatio){
        ...
        everyHeight = (int) (everyWidth * this.sizeRatio);
        int num = getChildCount();
        if(num > 0){
            for (int i = 0; i < num; i++) {

                FrameLayout fa = (FrameLayout) getChildAt(i);
                fa.getChildAt(0).getLayoutParams().width = everyWidth;
                fa.getChildAt(0).getLayoutParams().height = everyHeight;
                fa.measure(everyWidth, everyHeight);
                fa.requestLayout();
    ...
    //为了防止滑动item 过后 再次调用onLayout 后  会自动显示下一个item
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            int left = originX.get(i + fixIndex);
...
//                onRelease(event.getX(), 0);
    ...
    //自动上下切换
    private void autoChange(int temp , int endTemp,  int velocity){
        downX = temp;
        downY = temp;
        lastX = temp;
        scrollMode = MODE_IDLE;
        ...
        requireScrollChange(endTemp - temp);
        lastX = endTemp;


        recycleVelocityTracker();

        onRelease(endTemp, velocity);
    }

    public void next(){
        autoChange(120,100,-1900);
    }

    public void prev(){
        autoChange(100,120,1900);
    }
    ...
    public void updateHeight(int width, int height){
        for (int i = 0; i < getChildCount(); i++) {
            ...
            getChildAt(i).measure(width == 0 ? everyWidth : width, height== 0 ? everyHeight : height);
    ...
    private void doBindAdapter() {
        ...
        if (getChildCount() == 0) {
            ...
            for (int i = 0; i < 6; i++) {
                TFrameLayout frameLayout = new TFrameLayout(getContext());
                ...
                frameLayout.setListener(new TFrameLayout.ClickListener() {
                    @Override
                    public void onClick(View v) {
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < num; i++) {
            ...
            Log.d("asfas", "notifyDataSetChanged: i___" + i  + "tag : " + tag);
            if (i == 3 && tag == 0) {
                adapter.displaying(0);
            }
        }
